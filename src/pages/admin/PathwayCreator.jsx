...
const PathwayCreator = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 p-1 bg-slate-900/50 border border-slate-800 rounded-xl mb-8 w-fit">
        <button
          onClick={() => setActiveTab('basic')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'basic' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
        >
          <Layout size={16} /> Thông tin chung
        </button>
        <button
          onClick={() => setActiveTab('structure')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'structure' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
        >
          <Layers size={16} /> Cấu trúc nội dung
        </button>
        <button
          onClick={() => setActiveTab('settings')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'settings' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-300">Tiêu đề Pathway</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-300">Slug (URL)</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-24 pr-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                      value={form.slug}
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-300">Mô tả ngắn</label>
                <textarea
                  rows={4}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {form.tags.map(tag => (
                      <span key={tag} className="px-2 py-1 bg-slate-800 text-slate-300 text-xs rounded-md flex items-center gap-1">
                        {tag}
          ...
          {activeTab === 'structure' && (
            <div className="space-y-4">
              {form.courses.map((course, cIdx) => (
                <div key={course.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                  <div className="p-4 bg-slate-800/30 border-b border-slate-800 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => moveCourse(cIdx, -1)} disabled={cIdx === 0} className="p-2 text-slate-500 hover:text-indigo-400 disabled:opacity-30"><ArrowUp size={16} /></button>
                      <button onClick={() => moveCourse(cIdx, 1)} disabled={cIdx === form.courses.length - 1} className="p-2 text-slate-500 hover:text-indigo-400 disabled:opacity-30"><ArrowDown size={16} /></button>
                      <button
                  ...
                  <div className="p-6 space-y-6">
                    {course.modules.map((module, mIdx) => (
                      <div key={module.id} className="ml-8 border-l-2 border-slate-800 pl-6 space-y-4 relative">
                        <div className="absolute -left-[9px] top-0 w-4 h-4 bg-slate-900 border-2 border-slate-800 rounded-full"></div>

                        <div className="flex items-center gap-4">
                          <BookOpen size={18} className="text-slate-500" />
                          <input
                            className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-white text-sm font-semibold focus:ring-1 focus:ring-indigo-500 w-full"
                            value={module.title}
                            onChange={(e) => {
                              const newCourses = [...form.courses]
                          ...
                          <div className="flex items-center gap-1">
                            <button onClick={() => moveModule(cIdx, mIdx, -1)} disabled={mIdx === 0} className="p-1.5 text-slate-600 hover:text-indigo-400 disabled:opacity-20"><ArrowUp size={14} /></button>
                            <button onClick={() => moveModule(cIdx, mIdx, 1)} disabled={mIdx === course.modules.length - 1} className="p-1.5 text-slate-600 hover:text-indigo-400 disabled:opacity-20"><ArrowDown size={14} /></button>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-8">
                          {module.lessons.map((lesson, lIdx) => (
                            <div key={lIdx} className="flex items-center gap-2 group">
                              ...
                              <input
                                className="bg-transparent border-none text-slate-300 text-sm focus:ring-0 p-0 w-full hover:text-white"
                                value={lesson}
                    ...
                    <button
                      ...
                      className="ml-8 flex items-center gap-2 px-4 py-2 border-2 border-dashed border-slate-800 hover:border-indigo-500/50 text-slate-500 hover:text-indigo-400 rounded-xl text-sm transition-all"
                    >
              ...
              <button
                ...
                className="w-full py-4 border-2 border-dashed border-slate-800 hover:border-indigo-500/50 text-slate-500 hover:text-indigo-400 rounded-2xl flex items-center justify-center gap-2 font-semibold transition-all bg-slate-900/20"
              >
          ...
          {activeTab === 'settings' && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-8">
              <div className="flex items-center justify-between p-4 bg-slate-950 rounded-xl border border-slate-800">
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-lg ${form.isOfficial ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-400'}`}>
                    <ShieldCheck size={20} />
                  </div>
                  <div>
                    <h3 className="font-bold text-white">Lộ trình chính thức</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    type="text"
                    placeholder="Mặc định sử dụng tiêu đề Pathway"
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                  />
                </div>
                <div className="space-y-2">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sticky top-24">
            ...
            <div className="rounded-xl overflow-hidden bg-slate-950 border border-slate-800 aspect-video relative mb-4 group">
              {form.thumbnail ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${form.difficulty === 'beginner' ? 'bg-emerald-500/10 text-emerald-500' : form.difficulty === 'intermediate' ? 'bg-blue-500/10 text-blue-500' : 'bg-amber-500/10 text-amber-500'}`}>
                  {form.difficulty}
                </span>
                {form.isOfficial && (
                  <span className="px-2 py-0.5 bg-indigo-500/10 text-indigo-400 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
            ...
            {/* <div className="mt-6 p-4 bg-indigo-600/5 rounded-xl border border-indigo-500/20">
